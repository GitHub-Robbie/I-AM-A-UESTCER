...
void echo_rep(int sockfd)
{
    ...
    do
    {
        ...
        do
        {
            ...
            if(res < 0)
            {
                ...
                if(errno == EINTR)
                {
                    ...
                    continue;               //若是其他信号中断，则重新执行读取，跳出本轮while
                }
                free(buf);
                return;
            }
        ...
        {
            printf("[srv] fail to write len!\n");
            exit(1);
        }
        if(write(sockfd, buf, sizeof(char)*len) == -1)
        {
            printf("[srv] fail to write buf!\n");
            exit(1);
        }
        // TODO 发送结束，释放buf；
        free(buf);
...
int main(int argc,char *argv[])
{
    ...
    char srv_ip[32];
    (AF_INET, &srv_addr.sin_addr, srv_ip, sizeof(srv_ip));
    ...
    if(listenfd == -1)
    {
        printf("[srv] fail to get socket listenfd!\n");
        exit(1);
    }
    ...
    {
        printf("[srv] fail to bind socket!\n");
        exit(1);
    }
    ...
    while(!sig_to_exit)
    {
        ...
        // if(connfd == -1 && errno == EINTR && sig_type = SIG_INT)
        //     break;
