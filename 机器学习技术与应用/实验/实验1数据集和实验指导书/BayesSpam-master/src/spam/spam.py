...
class spamEmailBayes:
    ...
    def getStopWords(self):
        stopList=[]
        for line in open("../data/中文停用词表.txt", 'r', encoding='gbk', errors='ignore'):
    ...
    def getTestWords(self,testDict,spamDict,normDict,normFilelen,spamFilelen):
        ...
        after = sorted(wordProbList.items(),key=lambda d:d[1],reverse=True)[0:15]
        file = open("./wordprobList.txt", 'a')
        for i in range(len(after)):
            s = str(after[i]).replace('[', '').replace(']', '')  # 去除[],这两行按数据不同，可以选择
            s = s.replace("'", '').replace(',', '') + '\n'  # 去除单引号，逗号，每行末尾追加换行符
            file.write(s)
        file.close()
        # print("保存文件成功")
