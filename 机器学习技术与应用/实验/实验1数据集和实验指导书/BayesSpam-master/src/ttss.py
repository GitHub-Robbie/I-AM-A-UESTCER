...
for fileName in normFileList:
    wordsList.clear()
    for line in open("../data/normal/"+fileName, 'r', encoding='gbk', errors='ignore'):
        # 过滤掉非中文字符
...
for fileName in spamFileList:
    wordsList.clear()
    for line in open("../data/spam/"+fileName, 'r', encoding='gbk', errors='ignore'):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
...
for fileName in testFileList:
    ...
    wordsDict.clear()
    wordsList.clear()
    for line in open("../data/test/"+fileName, 'r', encoding='gbk', errors='ignore'):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
...
testOutput = open("./testOutput.txt", 'w', encoding="utf-8")

for i,ic in testResult.items():
    if ((int(i) <= 1000) and (int(ic) == 0)) or ((int(i) > 1000) and (int(ic) == 1)):
        print(i + "/" + str(ic) + " correct", file=testOutput)
    else:
        print(i + "/" + str(ic) + " wrong", file=testOutput)
