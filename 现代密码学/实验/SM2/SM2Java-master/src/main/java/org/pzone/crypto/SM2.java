...
public class SM2 {
	...
	private static int w = (int) Math.ceil(n.bitLength() * 1.0 / 2) - 1;	//ceil向上取整
	...
	public static void printHexString(byte[] b) {
		for (int i = 0; i < b.length; i++) {
			String hex = Integer.toHexString(b[i] & 0xFF);	//& 0xFF保持二进制补码的一致性
			if (hex.length() == 1) {
				hex = '0' + hex;
			}
...
//		 "128B2FA8 BD433C6C 068C8D80 3DFF7979 2A519A55 171B1B65 0C23661D 15897263".replace(" ", ""), 16);
		 ...
		 sm02.exportPublicKey(publicKey1, "publickey.pem");
		 sm02.exportPrivateKey(privateKey1, "privatekey.pem");
